defmodule ExFunky.Trial do
    ...
    def errors(reasons) when is_list(reasons) do {:errors, reasons} end
    ...
    ## Examples
        ...
        iex> ExFunky.Trial.match(nil, &(&1), &(&1))
        [:enoent]
  
        iex> ExFunky.Trial.match(42, &(&1), &(&1))
        42

        iex> ExFunky.Trial.match({:ok, 42}, &(&1), &(&1))
        42

        iex> ExFunky.Trial.match({:error, :enoent}, &(&1), &(&1))
        [:enoent]

        iex> ExFunky.Trial.match({:errors, []}, &(&1), &(&1))
        []

        iex> ExFunky.Trial.match({:errors, [:invalid_arg, :nul_dereferenced]}, &(&1), &(&1))
        [:invalid_arg, :nul_dereferenced]
  
    """
    @spec match(t,(value -> any), (list(reason) -> any)) :: any
    def match(trial, ok_fun, err_fun) do
    ...
    def is_ok(trial), do: trial |> match((fn _ -> true end), (fn _ -> false end))
    ...
    def is_error(trial), do: trial |> match((fn _ -> false end), (fn _ -> true end))
    ...
    ## Examples
        ...
        iex> ExFunky.Trial.error(nil) 
        ...> |> ExFunky.Trial.to_maybe()
        :none
  
    """
    @spec to_maybe(t) :: ExFunky.Maybe.t
    def to_maybe(trial), do: trial |> match(&ExFunky.Maybe.some/1, (fn _ -> ExFunky.Maybe.none() end))
    ...
    def get_value!(trial), do: trial |> match(&(&1), (fn _ -> throw :enoent end))
    
  
    # @doc """
    # Applies a given `binder` function to `x` if the given `Trial` is `{:some, x}`.
    # The result is given as such, unlike the `map/2` which wraps the result 
    # into a `:some` value.
  
    # Returns `:none` if the `Trial` is `:none`.
    ...
    #     iex> ExFunky.Trial.ok(42) 
    #     ...> |> ExFunky.Trial.bind(fn _ -> ExFunky.Trial.none() end)
    #     :none
